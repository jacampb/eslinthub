...
#def callRestireJS(config, row):

def callESlint(config, row):
...
def main():
	...
	#creates the connection, user name and password right now are just placeholders
	connection = mysql.connector.connect (**config)

	#adds cursor object
	cursor = connection.cursor()

	#executes the SQL query on ut_repos
